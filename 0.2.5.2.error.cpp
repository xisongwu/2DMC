...
int blocklv[101] = { 0, 2, 1, 2, 3, 1, 0, 0, 0, 2, 114, 0, 4, 0, 0, 0, 1, 0, 0, 0, 114514 };
...
int beibao[101][2];
bool run = 1;
int bed, back_x = 1, back_y = 50, Clocknum = 0, jump = 0, dx, dy, my, tall, ks = 0, mx, fz, cho = 1, key_ = 0, life = INT_MAX, bx, by, fell = 0, co = 0, H = 0, anx = 0, mpx, mpy, lx, ly, wjlv = 1, playerCD = 0, tick = 0, setfps = 1, gongji = 1, gongjicd = 20;
...
int now_time = 0,night = 0, will_night = 0, will_light = 0;
...
bool read()
{
    ...
    for (int i = 0; i < 500; i++)
    {
        if (fscanf(fp, " %lf %lf %lf %lf %lf %lf %d %d %d",
                   &shiti[i].HP, &shiti[i].hurt, &shiti[i].gx, &shiti[i].gy,
                   &shiti[i].spgy, &shiti[i].spgx, &shiti[i].type,
                   &shiti[i].jp, &shiti[i].CD) < 9)
        {
            // 如果读取失败（旧版存档），初始化怪物数据
            shiti[i].type = 0;
            shiti[i].HP = 0;
...
BOOL g_bClicking = FALSE;
DWORD g_dwInterval = 0; 
HANDLE g_hThread = NULL;
DWORD g_dwThreadId = 0;
DWORD WINAPI ClickingThread(LPVOID lpParam)
{
	INPUT input = {0};
...
void ToggleClicking()
{
	g_bClicking = !g_bClicking;
	if (g_bClicking)
	{
		g_hThread = CreateThread(NULL, 0, ClickingThread, NULL, 0, &g_dwThreadId);
		if (g_hThread == NULL)
		{
			MessageBox(NULL, TEXT("无法创建点击线程!"), TEXT("错误"), MB_ICONERROR);
			g_bClicking = FALSE;
		}
	}
	else
...
void SetClickInterval(DWORD dwInterval)
{
//	if (dwInterval >= 10) 
		g_dwInterval = 0;
}
...
//{
//	_tprintf(_T("鼠标连点器 - 使用Windows API实现\n"));
//	_tprintf(_T("--------------------------------\n"));
//	_tprintf(_T("快捷键:\n"));
//	_tprintf(_T("  F6 - 开始/停止点击\n"));
//	_tprintf(_T("  F7 - 增加点击间隔(+50ms)\n"));
//	_tprintf(_T("  F8 - 减少点击间隔(-50ms)\n"));
//	_tprintf(_T("  ESC - 退出程序\n\n"));
//	_tprintf(_T("当前状态: 停止 | 点击间隔: %d ms\n"), g_dwInterval);
//	while (TRUE)
//	{
//		if (GetAsyncKeyState(VK_F6) & 0x8000)
//		{
//			ToggleClicking();
//			_tprintf(_T("当前状态: %s | 点击间隔: %d ms\n"),
//			         g_bClicking ? _T("运行中") : _T("停止"), g_dwInterval);
//			Sleep(300);
//		}
//		if (GetAsyncKeyState(VK_F7) & 0x8000)
//		{
//			SetClickInterval(g_dwInterval + 50);
//			_tprintf(_T("当前状态: %s | 点击间隔: %d ms\n"),
//			         g_bClicking ? _T("运行中") : _T("停止"), g_dwInterval);
//			Sleep(300);
//		}
