...
int beibao[101][2];
bool run = 1;
int bed, back_x = 1, back_y = 50, Clocknum = 0, jump = 0, dx, dy, my, tall, ks = 0, mx, fz, cho = 1, key_ = 0, life = 20, bx, by, fell = 0, co = 0, H = 0, anx = 0, mpx, mpy, lx, ly, wjlv = 1, playerCD = 0, tick = 0, setfps = 1, gongji = 1, gongjicd = 20;
...
void Updateblock()
{
    if (s[dx][dy] == 11)
    {
        if (co !=-27  && !night && !will_light && !will_night)Color(-27), co = -27;
...
//	shuaxin();
	for(int i=0;i<_save.size();i++)
		if(GetAsyncKeyState(_save[i]));
        SetFileReadOnly(_save.c_str(), false);
        ...
        save();
        SetFileReadOnly(_save.c_str(), true);
        return 0;
    }
    catch (...)
    {
