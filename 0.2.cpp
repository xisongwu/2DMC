...
int beibao[101][2];
int Clocknum=0,jump=0,dx,dy,my,tall,ks=0,mx,fz,cho=1,key_=0,life=20,bx,by,fell=0,co=0,H=0,anx=0,mpx,mpy,lx,ly,wjlv=1;
...
void craft_update(int num)
{
	...
	for(int i=(j-10)/2*5+1;i<=(j-10)/2*5+5;i++)
	{
		...
		if(beibao[i][0]==12)cout<<beibao[i][1]<<"0\b×êÊ¯"<<"   ";
		if(beibao[i][0]==13)cout<<beibao[i][1]<<"0\bÌú¶§"<<"   ";
		if(beibao[i][0]==14)cout<<beibao[i][1]<<"0\bÌú¸å"<<"   ";
		if(beibao[i][0]==15)cout<<beibao[i][1]<<"0\b×êÊ¯¸å"<<"   ";
		if(beibao[i][0]==16)cout<<beibao[i][1]<<"0\b»ð°Ñ"<<"   ";}
		else cout<<" ÎÞ     ";
	}
	if(num==1){
	Setpos(6,7);cout<<" ºÏ³ÉÅä·½£º";
	Setpos(6,8);cout<<"  1Ô­Ä¾->2Ä¾¹÷";
	Setpos(6,9);cout<<"  2Ä¾¹÷+3Ô­Ä¾->Ä¾¸å";
	Setpos(6,10);cout<<"  2Ä¾¹÷+5Ô²Ê¯->Ê¯¸å";
	Setpos(6,11);cout<<"  20Ô²Ê¯+5ÃºÌ¿->ÈÛÂ¯";
	Setpos(6,12);cout<<"  2Ä¾¹÷+3Ìú¶§->Ìú¸ä";
	Setpos(6,13);cout<<"  2Ä¾¹÷+3×êÊ¯->×êÊ¯¸ä";
	Setpos(6,14);cout<<"  1Ä¾¹÷+1ÃºÌ¿->5»ð°Ñ";}
	else if(num==2){
	Setpos(6,7);cout<<" ÈÛÁ¶Åä·½£º";
	Setpos(6,8);cout<<"  1´ÖÌú+1ÃºÌ¿->1Ìú¶§";
	}
	}
...
int main()
{
	...
	for(int i=1;i<=9;i++)
	{
		...
		if(beibao[i][0]==12)cout<<" ×êÊ¯£º"<<beibao[i][1]<<"  ";
		if(beibao[i][0]==13)cout<<" Ìú¶§£º"<<beibao[i][1]<<"  ";
		if(beibao[i][0]==14)cout<<" Ìú¸å£º"<<beibao[i][1]<<"  ";
		if(beibao[i][0]==15)cout<<" ×êÊ¯¸å£º"<<beibao[i][1]<<"  ";
		if(beibao[i][0]==16)cout<<" »ð°Ñ£º"<<beibao[i][1]<<"  ";}
		else cout<<" ÎÞ  ";
	}
