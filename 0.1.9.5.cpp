...
char title[256];
int blockwj[101]={0,200,200,400,800,100,0,0,0,200,-1,0,1200,0};
int blocktouch[101]={0,1,0,1,1,1,0,0,0,0,1,0,1,0};
int blocklv[101]={0,2,1,2,3,1,0,0,0,2,1,114,4,0};
int beibao[101][2];
...
void setlight()
{
	...
	for(int j=1;j<=999;j++){
		...
		{
			light[i][j]=1;
			if(light[i-1][j+1]==0)light[i-1][j+1]=1;
			if(light[i-1][j-1]==0)light[i-1][j-1]=1;
			if(light[i+1][j+1]==0)light[i+1][j+1]=1;
			if(light[i+1][j-1]==0)light[i+1][j-1]=1;
			if(light[i][j+1]==0)light[i][j+1]=1;
			if(light[i][j-1]==0)light[i][j-1]=1;
			if(light[i+1][j]==0)light[i+1][j]=1;
			if(light[i-1][j]==0)light[i-1][j]=1;
		}
}
}
...
void Print()
{
	...
	for(int i=0;i<=50;i++)
	{
		...
		else if(painting[i][j][0]!=s[dx][dy]||painting[i][j][1]!=light[dx][dy]){
...
void craft_update(int num)
{
	...
	Setpos(7,12);cout<<"  2Ä¾¹÷+3Ìú¶§->Ìú¸ä";
	Setpos(7,13);cout<<"  2Ä¾¹÷+3×êÊ¯->×êÊ¯¸ä";}
	else if(num==2){
	Setpos(7,7);cout<<" ÈÛÁ¶Åä·½£º";
	Setpos(7,8);cout<<"  1´ÖÌú+1ÃºÌ¿->1Ìú¶§";
	}
	}
