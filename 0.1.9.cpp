...
int blockwj[101]={0,200,1,400,800,100,0,0,0,0,-1,0,0};
int beibao[101][2];
int Clocknum=0,jump=0,dx,dy,my,tall,ks=0,mx,fz,cho=1,key_=0,life=20,bx,by,fell=0,co=0,H=0,anx=0,mpx,mpy,lx,ly;
double jx=1,jy=50,x=1,y=50,g=1,spx=0,spy=0,wj=0;
...
void time(){
	now_time++;
	if(now_time==1080) will_night=1,sky();
	if(now_time==1440) night=1,will_night=0,sky();
	if(now_time==2520) will_light=1,sky();
	if(now_time==2880) night=0,now_time=0,will_light=0,sky();
}
void MAP()
{
	int a,moun=0;
...
void wajue(int sx,int sy)
{
	...
	if(s[sx][sy]==2){
	if(wj>200){int U=getbeibao(s[sx][sy]);
...
void craft_update()
{
	...
	{
		if(beibao[i][1]>0){
		if(beibao[i][0]==1)cout<<beibao[i][1]<<"0\b圆石"<<"   ";
		if(beibao[i][0]==2)cout<<beibao[i][1]<<"0\b原木"<<"   ";
		if(beibao[i][0]==3)cout<<beibao[i][1]<<"0\b煤炭"<<"   ";
		if(beibao[i][0]==4)cout<<beibao[i][1]<<"0\b粗铁"<<"   ";
		if(beibao[i][0]==5)cout<<beibao[i][1]<<"0\b泥土"<<"   ";
		if(beibao[i][0]==6)cout<<beibao[i][1]<<"0\b木棍"<<"   ";
		if(beibao[i][0]==7)cout<<beibao[i][1]<<"0\b木稿"<<"   ";
		if(beibao[i][0]==8)cout<<beibao[i][1]<<"0\b石稿"<<"   ";
		if(beibao[i][0]==9)cout<<beibao[i][1]<<"0\b熔炉"<<"   ";}
		else cout<<" 无     ";
	}
	Setpos(7,7);cout<<" 合成配方：";
	Setpos(7,8);cout<<"  1原木->2木棍";
	Setpos(7,9);cout<<"  2木棍+3原木->木稿";
	Setpos(7,10);cout<<"  2木棍+5圆石->石稿";
	Setpos(7,11);cout<<"  20圆石+5煤炭->熔炉";
	}
...
int main()
{
	...
	for(int i=1;i<=9;i++)
	{
		cout<<i<<".";
		if(beibao[i][1]>0){
		if(beibao[i][0]==1)cout<<" 圆石："<<beibao[i][1]<<"  ";
		if(beibao[i][0]==2)cout<<" 原木："<<beibao[i][1]<<"  ";
		if(beibao[i][0]==3)cout<<" 煤炭："<<beibao[i][1]<<"  ";
		if(beibao[i][0]==4)cout<<" 粗铁："<<beibao[i][1]<<"  ";
		if(beibao[i][0]==5)cout<<" 泥土："<<beibao[i][1]<<"  ";
		if(beibao[i][0]==6)cout<<" 木棍："<<beibao[i][1]<<"  ";
		if(beibao[i][0]==7)cout<<" 木稿："<<beibao[i][1]<<"  ";
		if(beibao[i][0]==8)cout<<" 石稿："<<beibao[i][1]<<"  ";
		if(beibao[i][0]==9)cout<<" 熔炉："<<beibao[i][1]<<"  ";}
		else cout<<" 无  ";
	}
